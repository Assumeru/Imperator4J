...
@Endpoint(mode = Mode.GAME, type = "autoroll")
public class SetAutoRoll {
	...
	public JSONObject handle(Member member, @Param("gid") int gid, @Param("autoroll") boolean autoroll) throws RequestException, TransactionException {
		...
		if(game == null) {
			throw new InvalidRequestException("Game does not exist", Mode.GAME, "autoroll");
		}
		Player player = game.getPlayerById(member.getId());
		if(player == null) {
			throw new InvalidRequestException("Player not in game", Mode.GAME, "autoroll");
		}
